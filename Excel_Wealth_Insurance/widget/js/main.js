;(function(window, undefined) {    
  ...
  window.addEventListener("message", (event) => {
    if (event.origin !== "https://train.alteirac.com")
      return;
      var vizzes=tableau.VizManager.getVizs();
      for(b=0;b<vizzes.length;b++){
        workbook = vizzes[b].getWorkbook();
        activeSheet = workbook.getActiveSheet();
        activeSheet.applyFilterAsync("Device",event.data,tableau.FilterUpdateType.REPLACE);
      }
      curFilters={"Parts":[event.data]}
  }, false);
  window.onresize = function(e){
    ...
    doit = setTimeout(function(){resizeVizzes();if(!e.fake)$(".gridster").find(".mask").fadeOut(MASK_DELAY);}, 3000);
  };
  ...
  function resizeVizzes(){
    ...
    for(b=0;b<vizzes.length;b++){
      ...
      try {
        ...
        if(viz.getUrl().indexOf("KPI")!=-1)viz.setFrameSize(w,h); 
      } catch (error) {
    ...
    gridster = $(".gridster ul").gridster({
      ...
      draggable: {
          ...
          start: function (e, ui, $widget) {
            highLight();
            $(".tab-widget").css("opacity","0.01");
            // ui.$helper.find(".tab-widget").hide();
          },
          stop: function (e, ui, $widget) {
            $(".tab-widget").css("opacity","1");
            setTimeout(() => {
  ...
  function _expandWidget(widget){
    ...
    if(widget.hasClass("maximized")) {
      widget.find(".tableauPlaceholder").css("opacity","1");
      widget.find(".tableauPlaceholder").css("z-index","90");
      widget.find(".tableauPlaceholderNOPKI").css("z-index","80");
      widget.find(".tableauPlaceholderNOKPI").css("opacity","0.01");
      //activate the auto resize
      $(".grid ul").removeClass("desac");
      ...
      //deactivate the auto resize
      $(".grid ul").addClass("desac");
      $(".grid ul").css("height","865px");
      ...
      $(".maximized").attr("style",$(".maximized").attr("style")+"height:865px!important");
      $('.maximized').on('scroll',()=>{
        console.log("scroll fixed...")
        $('.maximized').scrollTop(0);
      })
      widget.find(".tableauPlaceholder").css("opacity","0.01");
      widget.find(".tableauPlaceholderNOKPI").css("opacity","1");
      widget.find(".tableauPlaceholder").css("z-index","80");
      widget.find(".tableauPlaceholderNOKPI").css("z-index","90");
      gridster.disable();
      gridster.disable_resize();
      setTimeout(() => {
        widget.find(".mask").fadeOut(MASK_DELAY);
  ...
  function getFirstWidgetSize(){
    ...
    return {"x":mobileCheck()==true?15:15,"y":5}  
  }
  function addWidgetToGrid(chartID){
    curNew=getUniqueID();
    gridster.add_widget.apply(gridster, [`<li id="${curNew}" style="margin-top: auto; margin-bottom: auto; min-height: auto;">NEW</li>`, 30, 8,1,1,null,false]);
    var nid=getUniqueID()
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        loadVizInit(chartID.replaceAll("KPI",""),nid+'_NOKPI',true);
      }, 1000);
    }, 1000);
    editMode(true);
    packGrid();
  ...
  function restoreGrid(){
    ...
    arrNode.map((e)=>{
      setTimeout(() => {
        ...
        loadVizInit(e.chartName.replaceAll("KPI",""),e.vizID+'_NOKPI',true);
      }, 1000);
    })
  }
  ...
  function getNodeTemplate(genID,chartName){
    var tp=`
        ...
        <div class="tab-widget">
          <div class='tableauPlaceholderNOKPI' chart="${chartName}" id='${genID}_NOKPI' style='width: 100%; height: 100%'></div>
          <div class='tableauPlaceholder' chart="${chartName}" id='${genID}' style='width: 100%; height: 100%'></div>
          <div class="mask">
  ...
  function loadVizInit(url,id,max) {
    ...
    var optView = {
      ...
      width: max?$(placeholderView).width()*2:$(placeholderView).width(),
      height: max?"893px":$(placeholderView).height(),
      // width: "100%",
      // height: "100%",
      hideTabs: true,
      hideToolbar: true,
      device:mobileCheck()==true?"mobile":"desktop"
    };
  ...
  function resetFilters(){
    ...
    for(b=0;b<vizzes.length;b++){
      ...
      // workbook = vizzes[b].getWorkbook();
      // activeSheet = workbook.getActiveSheet();
      // if(activeSheet.getSheetType()==tableau.SheetType.DASHBOARD && activeSheet.getWorksheets()[0])
      //   activeSheet.getWorksheets()[0].clearSelectedMarksAsync(); 
      // else{
      //   if(activeSheet.clearSelectedMarksAsync) 
        ...
        for(b=0;b<vizzes.length;b++){
          ...
          if(true || curWB==workbook.getName()){
            ...
            for(var i in mk){
              ...
              if(mk[i] && mk[i][0] && activeSheet.getName()=="3dModel")
                if(mk[i][0]=="Hide")
                  workbook.changeParameterValueAsync("changeme", "two")
                else
                  workbook.changeParameterValueAsync("changeme", "one")
            }
          }
        }
      if(!$.isEmptyObject(mk)){
  ...
  function setMenuState(){
    ...
    $(".askFeat").attr("title","Demander aux Données...");
    $(".editFeat").attr("title","Edition...");
    if(askurl==null){
      $(".askFeat").addClass("disable");
      $(".askFeat").attr("title","Configurer ASK DATA URL dans la configuration");
    }
    if(edurl==null){
      $(".editFeat").addClass("disable");
      $(".editFeat").attr("title","Configurer WEB EDIT URL dans la configuration");
    }
  }
