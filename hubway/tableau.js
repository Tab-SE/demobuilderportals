...
function onMarksSelection(marksEvent) {
  getIndexFromViz(viz).then(function (index) {
    if (findElement(tab_action, index) && findElement(tab_action, index).val == "true")
      return marksEvent.getMarksAsync().then(reportSelectedMarks, (err) => {
        alert("You don't have right to download data thus not able to see marks. Uncheck 'Actions' in the view settings")
      });
  })
}
...
//ESKI: 2022 Gartner MQ Event
let vizIndex = 0;
const timerActions = 10000;
const timerDataRefresh = 15000;
const waitTimeMarkSelection = 2000;
const waitTimeMarkSelectioncleared = 6000;

function gartnerShowTime() {
  //workbook Actions timer
  setInterval(() => gartnerActions(vizIndex), timerActions);

  //Data refresh timer
  setInterval(() => viz.refreshDataAsync(), timerDataRefresh);
}

function gartnerActions(index) {
  sheetName = workbook.getPublishedSheetsInfo()[index].getName();
  console.log(vizIndex, sheetName);

  //switch tab
  workbook.activateSheetAsync(sheetName).then(function doSomething(sh) {
    //...then wait 2 seconds and select mark and 6 seconds to unselect it
    if (sh.getName() == 'Map') {

      setTimeout(() => sh.selectMarksAsync('STATION_NAME', 'Somerville City Hall',
        tableau.SelectionUpdateType.REPLACE), waitTimeMarkSelection);

      setTimeout(() => sh.clearSelectedMarksAsync(), waitTimeMarkSelectioncleared);
    }

  });

  //increase vizIndex for timer function
  if (vizIndex < workbook.getPublishedSheetsInfo().length - 1) {
    // index for next viz
    vizIndex++
  } else {
    //index for first viz
