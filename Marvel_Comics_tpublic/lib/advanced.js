...
function showFilterBox(el) {
  ...
  document.querySelectorAll(`.dropdown-content`).forEach((dd) => {
    if (dd.getAttribute("mid") != el) dd.classList.remove("show");
  });
}
function populateFilterMenu(fil) {
  ...
  fil.getAppliedValues().map((val) => {
    var found = false;
    document
      .querySelectorAll(`[filName="${fil.getFieldName()}"]`)
      .forEach((el) => {
        console.log();
        if (el.text == val.value.toString()) found = true;
      });
    if (!found) {
      links += `<a filName="${fil.getFieldName()}" href="#" class="filter-entry" onclick="applyFilter('${fil.getFieldName()}','${
        val.value
      }')">${val.value}</a>`;
    }
  });
  ...
  if (document.querySelector(`div[mid='${fil.getFieldName()}']`) == null) {
    document.getElementsByClassName("filter-container")[0].innerHTML =
      list + document.getElementsByClassName("filter-container")[0].innerHTML;
  } else {
    document.querySelector(`div[mid='${fil.getFieldName()}']`).innerHTML +=
      links;
  }
}
function populateParameterMenu(param) {
  ...
  param.getAllowableValues().map((val) => {
    var found = false;
    document
      .querySelectorAll(`[paramName="${param.getName()}"]`)
      .forEach((el) => {
        if (el.text == val.value) found = true;
      });
    if (!found) {
      links += `<a paramName="${param.getName()}" href="#" class="filter-entry" onclick="applyParam('${param.getName()}','${
        val.formattedValue
      }')">${val.formattedValue}</a>`;
    }
  });
  var list = `
  <span class="filter_dropdown">
    ...
    if (typeof tab_all_filters != "undefined") {
      ...
      current_filter.map((fl) => {
        ...
        tab_all_filters[index].filters.map((tb) => {
          if (tb.getFieldName() == fl.getFieldName()) found = true;
        });
        if (found == false) tab_all_filters[index].filters.push(fl);
      });
...
function setViewMenuVisibility() {
  ...
  tab_server.map((s, ind) => {
    if (s == "") {
      ...
      document.querySelector(`.viz[index='${ind + 1}']`).style.display =
        "inline-flex";
      firstIndex.push(ind);
    }
  });
...
function getIndexFromViz(viz) {
  return new Promise((resolve, reject) => {
    viz.getCurrentUrlAsync().then(function (current_url) {
      var index = tab_server.findIndex((element) =>
        element.includes(current_url.split("?")[0])
      );
      resolve(index);
    });
  });
...
function findElement(arr, keyVal) {
  var found = arr.filter(function (item) {
    return item.key === keyVal.toString();
  });
  return found[0] || null;
}
function initialize() {
  if (typeof tab_img != "undefined") {
    restoreImgs();
    restoreTexts();
    if (title_index) document.querySelector("head title").text = title_index;
  }
}
