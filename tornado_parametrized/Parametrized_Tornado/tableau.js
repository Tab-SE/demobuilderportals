...
function loadVizByIndex(index, force, device = "") {
  ...
  if (!isSameWorkbook) {
    ...
    options = {
      ...
      "Parameter Table 1": "Customer Name",
      "Parameter Table 2": "Region",
      "Parameter Size by": "CAMPO NOVO",
    };
    if (url) {
...
function launchAsk() {
  ...
  viz.getCurrentUrlAsync().then(function (current_url) {
    ...
    loadViz(
      containerDiv,
      tab_ask[getElementIndexByIndex(tab_ask, indexv)].val,
      ask_options
    );
  });
}
...
function launchAction() {
  ...
  if (textOnly.length == 0)
    window.open(
      "http://google.com/search?q=There are no text values in your selection :-)"
    );
  if (lengthInUtf8Bytes(textOnly.join(" ")) < 1024)
    window.open(
      "http://google.com/search?q=" + encodeURIComponent(textOnly.join(" "))
    );
  else
    window.open(
      "http://google.com/search?q=" +
        "Too many elements in your selection :-) Please reduce !"
    );
}
function onMarksSelection(marksEvent) {
  getIndexFromViz(viz).then(function (index) {
    if (
      findElement(tab_action, index) &&
      findElement(tab_action, index).val == "true"
    )
      return marksEvent.getMarksAsync().then(reportSelectedMarks, (err) => {
        alert(
          "You don't have right to download data thus not able to see marks. Uncheck 'Actions' in the view settings"
        );
      });
  });
}
